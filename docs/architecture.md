...
7. Project Pi events into either stable stdout/stderr output or Senko's Pi-TUI-based interactive view.
...
Senko does not perform model discovery, update checks, telemetry, provider fallback, or hidden request retries during
startup. Performance measurements are informational in milestone 1; stable budgets will be set after a production
